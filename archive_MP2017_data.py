...
parz = 'G:/MP17_archive'
os.chdir(par)
logfile = '%s/MP17_archive.log' % parz
...
for g in Gs:
    if g[0] == 'G':
        for s in ['S01','S03','S04']:
            ...
            dirs2zip = []
            dirs2zip.append('land_area_time_series_plots')
            ...
            for bi in ['ST73124','ST73126','ST73129','ST73131','ST73139','ST73141']:
                for y in range(1,51):
                    files2zip.append('bimode/%s/results/profile_%04d' % (bi,y) )
            ...
            for bi in ['ST73124','ST73126','ST73129','ST73131','ST73139','ST73141']:
                os.makedirs('%s/%s/bimode/%s/results' % (parz,sub,bi),exist_ok=True )
            ...
            for f2z in files2zip:
                ...
                try:
                    ...
                    arcfname = orig.split('/')[-1]      # store just file name to drop all directories from archive structure
                    with zipfile.ZipFile(arc,mode='w',compression=zipfile.ZIP_DEFLATED) as farc:
                        farc.write(orig,arcfname)
                    zip_success = 1  
                    if zip_success == 1:
                        try:
                            os.remove(orig)
