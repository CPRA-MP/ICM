...
plot_FFIBS      = True
vfpy_exe = '/ocean/projects/bcs200002p/ewhite12/code/ICM/postprocess/ICM_FFIBS_mapping.py'

plot_IC_change  = True
plot_FWOA_diff  = False
mdpy_exe = '/ocean/projects/bcs200002p/ewhite12/code/ICM/postprocess/ICM_morph_differencing.py'

plot_IC_FWOA_diff  = False
mdpy3_exe = '/ocean/projects/bcs200002p/ewhite12/code/ICM/postprocess/ICM_morph_differencing_3ras.py'

plot_hydro_ts   = True
htpy_exe = '/ocean/projects/bcs200002p/ewhite12/code/ICM/postprocess/ICM-Hydro_plotting_noCRMS.py'

plot_veg_ts     = True
vtpy_exe = '/ocean/projects/bcs200002p/ewhite12/code/ICM/postprocess/ICM-Veg_timeseries_plotting.py'
vtepy_exe = '/ocean/projects/bcs200002p/ewhite12/code/ICM/postprocess/ICM-Veg_ecoregion_timeseries_plotting.py'
...
for y in years:
    ...
    if plot_TIFs == True:
        ...
        cmd1 = 'python %s %s %s %s %s\n' % (tmpy_exe,s,g_fwa,y,sy)
    else:
        cmd1 = ''

    if plot_FFIBS == True:
        print('   - mapping LAVegMod FFIBS outputs as TIFs/PNGs')
        cmd2 = 'python %s %s %s %s %s\n' % (vfpy_exe,s,g_fwa,y,sy)
    else:
        cmd2 = ''

    if plot_IC_change == True:
        if ey > ey_ic:
            cmd3 = 'python %s %s %s %s %s %s %s %s\n' % (mdpy_exe, s, g_fwa, g_fwa, ey, ey_ic, 'lndtyp30', 'lndchg')
        else:
            cmd3 = ''
    else:
        cmd3 = ''
    ...
    if plot_FWOA_diff == True:
        ...
        for difftype in diffrastypes.keys():
            ...
            cmd4 = 'python %s %s %s %s %s %s %s %s\n' % (mdpy_exe, s, g_fwa, g_fwoa, ey, ey, rastype, difftype)
    ...
    if plot_IC_FWOA_diff == True:
        ...
        cmd5 = 'python %s %s %s %s %s %s %s %s %s\n' % (mdpy3_exe, s, g_fwa, g_fwoa, g_fwoa, ey, ey_ic, 'lndtyp30', 'lndtypdiff')
...
if plot_hydro_ts == True:
    ...
    ctrl_str = 'python %s %s %s\n' % (htpy_exe,s,g_fwa)
    sbatch_file_uniqueH = 'S%02d.G%03d.hydTS.%s' % (s,g_fwa,sbatch_file)
...
if plot_veg_ts == True:
    ...
    cmd1 = 'python %s %s %s %s\n' % (vtpy_exe,s,g_fwa,last_year)
    cmd2 = 'python %s %s %s %s\n' % (vtepy_exe,s,g_fwa,last_year)
    ctrl_str = cmd1 + cmd2
