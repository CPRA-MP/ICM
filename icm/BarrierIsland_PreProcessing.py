...
def BIyearlyVars(bimode_dir, BITI_Terrebonne_comp, BITI_Barataria_comp, BITI_Pontchartrain_comp, 
    ...
    # Initialize MHW array to zero - will write over previous year's array
    ...
    #   Do not allow inlets that are currently jettied to change in width dimension 
    #   All areal adjustments for these two links must occur in the depth dimension
    CamPassLinkID = 529
    ...
    # Set upper and lower limits on how much the tidal inlet dimensions can 
    #   change as a ratio of the initial dimensions
    # Currently, the limits are (arbitrarily) set to limit expansion to twice the 
    #   original width and/or depth and limit the contraction to 1/4th the original 
    #   width and/or depth
    for BITI_Link_ID in BITI_inlet_dimensions.keys():
        ...
        # Based on analysis by BITI team, when basin is completely open water, maximum increase 
        #  in inlet dimensions was 145% increase from initial dimensions
        BITI_inlet_depth_max = 1.5*orig_depth          
        ...
        # Based on analysis by BITI team, when basin is completely open water, maximum increase 
        #  in inlet dimensions was 145% increase from initial dimensions
        BITI_inlet_width_max = 1.5*orig_width           
        ...
        EHLinksArray[EHLinks_index,8] = BITI_MWL - new_depth
        # Channel width is attribute4 for Type 1 links (column 11 in links array)               
        EHLinksArray[EHLinks_index,11] = new_width                          


    
    return(IslandMHWCompLists, BIMODEmhw)
