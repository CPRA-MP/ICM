...
#     api variable main type |
#     ffibs_coverage_ratio   | 

#     api variable subtype   |    output column
#     ffibs_type = pl_bf     |    pct_vglnd_BLHF
#     ffibs_type = pl_bm     |    pct_vglnd_BM
#     ffibs_type = pl_fm     |    pct_vglnd_FM
#     ffibs_type = pl_im     |    pct_vglnd_IM
#     ffibs_type = pl_sf     |    pct_vglnd_SWF
#     ffibs_type = pl_sm     |    pct_vglnd_SM

ffibs_col_map = {'pct_vglnd_BLHF': 'pl_bf',
                 'pct_vglnd_BM': 'pl_bm',
                'pct_vglnd_FM': 'pl_fm',
                'pct_vglnd_IM': 'pl_im',
                'pct_vglnd_SWF': 'pl_sf',
...
vars_to_process= ['ffibs_coverage_ratio']
validate=True

#mimic behavior of api functionality
def kwargs_function(**kwargs):
    return kwargs

def postprocess_ffibs_cvg(year,base_icm,model,grid_version,start_year,sterm,gterm):
    ...
    var_time_unit = var_properties.time_units[0] #this only works with time units of one type 'annual' etc.
    ...
    centers_df = centers_df.rename(columns={centers_df.columns[0]: "cellid"}) #rename first col for join later
    
    sterm_str = sterm[1:]
    gterm_str = gterm[1:]
    
    for ffibs in ffibs_col_map.values():
        print(f"Begin processing {ffibs}")
        ...
        output_df = output_df.rename(columns=ffibs_col_map)
        output_df.columns = output_df.columns.str.lower()
        output_df = output_df.rename(columns={output_df.columns[0]: "cellid"}) #rename first column like centers_df
        ...
        species_values = spec_coords[ffibs].values
        ...
        #create empty raster
        raster = np.full((grid.height, grid.width),-9999,dtype=np.float32) #based on testing used np datatype, not polars
        ...
        dst_profile = {
            ...
            "crs": grid.crs,
            "driver": "COG",
            "dtype": np.float32, #based on testing used np datatype, not polars 
            "height": grid.height,
            "nodata": -9999,
            "resampling": "NEAREST",
