...
#########################################################################
comp_CRMS_map = r'D:\ICM\CRMS_ICM_compartment_map.csv'
scn = 'Hindcast (S00)'
grp = 'Calibration (G000)'
file_pre = ''	# prefix for run from file naming convention
start_day = dt.date(2012,1,1)
ICM_Hydro_dir = r'D:\ICM\ICM_hydro_CP'
CRMS_dir = r'D:\CRMS\CRMS_daily'
...
crms_cols = {'STG':8,'SAL':2}
y_axes_text = {'STG':r'Daily Mean Stage (m, NAVD88)','SAL':r'Daily Mean Salinity (ppt)'}
y_axes_range = {'STG':[-2.0, 2.0],'SAL':[0,36]}
CRMS_2_model_unit_conv_multipliers = {'STG':0.3048,'SAL':1.0}

for out_type in ['STG','SAL']:
	...
	# set output-specific variables
	CRMS_date_col = 1
	CRMS_column = crms_cols[out_type]
	y_txt = y_axes_text[out_type]
	y_range = y_axes_range[out_type]
	CRMS_2_model_unit_conv = CRMS_2_model_unit_conv_multipliers[out_type]
	
	if ncomps < len(all_comps):
		sys.exit('\n\n  !!!\n\n  Data check shows that there are more compartments listed in Links.csv than there is output data for in timeseries *.out files.\n\n  !!!\n\n Aborting plotting script.\n\n')

	nc = 0
	for c in range(0,ncomps):
		nc += 1
		comp = c+1
		print(r' - plotting %s - compartment %04d - %d/%d - %0.1f%%' %(out_type,comp,nc,ncomps,100*nc/ncomps))
		...
		try:
			...
			for row in f:
				if row[1] != 'na':
					...
					CRMS_vals.append(float(row[1])*CRMS_2_model_unit_conv)
		...
		plt.close()	
#
print (' done plotting timeseries')
#############################################################################################################
...
for c in range(0,ncomps):
	nc += 1
	comp = c+1	
	png_fname_stg = r'\STG\ICM-Hydro_comp%04d_STG.png' % (comp)
	png_fname_sal = r'\SAL\ICM-Hydro_comp%04d_SAL.png' % (comp)
	...
	with open(html_file,mode='w') as hf:
		...
		hf.write(' </head>')
		hf.write(' <body>')
		hf.write(' <p><img src="..\png\%s" align ="top"> <img src="..\png\%s" align ="top"></p>' % (png_fname_stg,png_fname_sal))
