...
inputs = np.genfromtxt('ICM_gadwall_fix_control.csv',dtype=str,comments='#',delimiter=',')
...
gadwall_fix_exe_path = inputs[9,1].lstrip().rstrip() # path to morph executable - ** path is relative to the S##/G###              ** - use './' if running copy of executable that is saved in /G## directory
...
for year in range(startyear+elapsed_hotstart,endyear_cycle+1):
    ...
    wm_param_file = r'%s/gadwall_fix_input_params.csv' % wetland_morph_dir
