...
#switch exe code#            os.rename(newfile,oldfile)
   ...
   # read in monthly TSS data
    ...
    with open (wm_param_file, mode='w') as ip_csv:
        ...
        ip_csv.write("'geomorph/input/MP29_ecoregion_shallow_soil_factor_mm.csv', ssub_file - file name with relative path to shallow subsidence table with statistics by ecoregion (mm/yr; positive values are for downward VLM)\n")
        ...
        ip_csv.write("'geomorph/input/MP29_ecoregion_organic_matter_accum.csv', eco_omar_file - file name with relative path to lookup table of organic accumulation rates by marsh type/ecoregion\n")
        ip_csv.write("'geomorph/input/MP29_compartment_ecoregion.csv', comp_eco_file - file name with relative path to lookup table that assigns an ecoregion to each ICM-Hydro compartment\n")
